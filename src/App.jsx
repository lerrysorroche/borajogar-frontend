...
function App() {
  ...
  const [mudarSenhaNovaConfirmacao, setMudarSenhaNovaConfirmacao] =
    useState("");
  ...
  const [periodoFiltroEstatisticas, setPeriodoFiltroEstatisticas] =
    useState("mes");
  ...
  useEffect(() => {
    ...
    if (pixPendente) {
      intervalId = setInterval(() => {
        fetch(
          `https://borajogar-api.onrender.com/recarga/status/${pixPendente.payment_id}`,
        )
          .then((res) => res.json())
          .then((data) => {
            if (data.status === "PAGO") {
              mostrarToast(
                "✅ Pagamento Confirmado! Saldo liberado.",
                "sucesso",
              );
  ...
  const abrirConfirmacao = (
    ...
    if (!usuarioLogado) {
      mostrarToast(
        "Faça login ou crie uma conta grátis para alugar jogos!",
        "aviso",
      );
      setModoLogin(true);
      setModoEsqueciSenha(false);
  ...
  const confirmarTransacao = () => {
    ...
    if (modalConfirmacao.tipo === "aluguel") {
      executarAluguel(
        modalConfirmacao.jogoId,
        precoFinal,
        modalConfirmacao.diasEscolhidos,
      );
    } else {
      executarReserva(
        modalConfirmacao.jogoId,
  ...
  const devolverAntecipado = (locacaoId, dataFim) => {
    ...
    let msg =
      "Atenção: Tem certeza que deseja devolver este jogo agora e encerrar seu acesso?";
    if (diasRestantes > 0) {
  ...
  const buscarDadosDoJogo = async () => {
    ...
    try {
      ...
      if (dadosBusca.results && dadosBusca.results.length > 0) {
        ...
        if (dadosDetalhes.playtime)
          setNovoJogoTempo(`${dadosDetalhes.playtime}h`);
        ...
        if (dadosDetalhes.description_raw) {
          ...
          try {
            ...
            const descricaoPortugues = dadosTraducao[0]
              .map((item) => item[0])
              .join("");
  ...
  const votarEnquete = (opcaoId) => {
    if (!usuarioLogado) {
      mostrarToast(
        "Você precisa criar uma conta grátis ou fazer login para votar!",
        "aviso",
      );
  ...
  const carregarDados = () => {
    ...
    if (usuarioLogado && usuarioLogado.id)
      urlEnquete += `?usuario_id=${usuarioLogado.id}`;
    fetch(urlEnquete)
    ...
    if (usuarioLogado.is_admin) {
      ...
      fetch("https://borajogar-api.onrender.com/admin/manutencao", {
        ...
        .then((res) => (res.ok ? res.json() : []))
        .then((dados) =>
          setContasManutencao(Array.isArray(dados) ? dados : []),
        );
    ...
    fetch(
      `https://borajogar-api.onrender.com/meus-alugueis/${usuarioLogado.id}`,
    )
      .then((res) => (res.ok ? res.json() : []))
      .then((dados) => setMeusAlugueis(Array.isArray(dados) ? dados : []));
    fetch(
      `https://borajogar-api.onrender.com/minhas-reservas/${usuarioLogado.id}`,
    )
      .then((res) => (res.ok ? res.json() : []))
    ...
    fetch(
      `https://borajogar-api.onrender.com/usuarios/${usuarioLogado.id}/saldo`,
    )
      ...
      .then((data) => {
        if (data && data.saldo !== undefined) {
          ...
          const userStorage = JSON.parse(
            localStorage.getItem("usuarioBoraJogar"),
          );
          if (userStorage) {
  ...
  const manterReserva = (notificacaoId) => {
    fetch("https://borajogar-api.onrender.com/notificacoes/ler", {
      ...
      mostrarToast(
        "Perfeito! Acompanhe a nova data em Minhas Reservas.",
        "sucesso",
      );
      carregarDados();
    });
  ...
  const salvarEdicaoCliente = (e) => {
    ...
    fetch(
      `https://borajogar-api.onrender.com/usuarios/${modalEdicaoCliente.id}`,
      {
        method: "PUT",
        headers: getAuthHeaders(),
        body: JSON.stringify({
          ...
          motivo_ajuste:
            modalEdicaoCliente.motivo_ajuste || "Ajuste Administrativo", // 🚀 Envia o motivo
        }),
      },
    )
      .then(async (res) => {
  ...
  const revogarLocacao = (locacaoId) => {
    if (
      ...
      fetch(
        `https://borajogar-api.onrender.com/admin/locacoes/${locacaoId}/revogar`,
        {
          method: "POST",
          headers: getAuthHeaders(),
        },
  ...
  const filtradosBase = jogos
    .filter((jogo) =>
      jogo.titulo.toLowerCase().includes(termoBusca.toLowerCase()),
    )
    .filter((jogo) => {
  ...
  const jogosFuturos = filtradosBase
    .filter((j) => {
      const dataLanc = j.data_lancamento
  ...
  const normaisDaPagina = jogosNormais.slice(
    indicePrimeiroJogo,
    indiceUltimoJogo,
  );
  ...
  const locacoesAtivasFiltradas = todasLocacoes
    ...
    .sort((a, b) => {
      if (ordenacaoLocacoes === "expira_breve")
        return new Date(a.data_fim) - new Date(b.data_fim);
      if (ordenacaoLocacoes === "expira_longe")
        return new Date(b.data_fim) - new Date(a.data_fim);
      if (ordenacaoLocacoes === "az_cliente")
  ...
  const reservasAdminFiltradas = todasReservas
    ...
    .sort((a, b) => {
      ...
      if (ordenacaoReservaAdmin === "az_cliente")
        return a.cliente.localeCompare(b.cliente);
      return 0;
    });
  ...
  const cancelarReservaAdmin = (reservaId, nomeCliente, tituloJogo) => {
    if (
      ...
      fetch(
        `https://borajogar-api.onrender.com/admin/reservas/${reservaId}/cancelar`,
        {
          method: "POST",
          headers: getAuthHeaders(),
        },
    ...
    .filter(
      (c) =>
        ...
        (c.ultimo_cliente_nome &&
          c.ultimo_cliente_nome
            .toLowerCase()
            .includes(buscaManutencao.toLowerCase())),
    )
    .sort((a, b) => {
      if (ordenacaoManutencao === "urgente")
        return (b.cashback_pendente || 0) - (a.cashback_pendente || 0);
      if (ordenacaoManutencao === "az_jogo")
        return a.jogo.localeCompare(b.jogo);
      return 0;
    });
  ...
  const jogosCatalogoAdminFiltrados = jogos
    ...
    .filter((j) => {
      ...
      if (filtroStatusCatalogo === "lancamentos")
        return idsLancamentos.includes(j.id);
      return true;
    });

  const alugueisAtivos = meusAlugueis.filter((item) => item.status === "ATIVA");
  const todosAlugueisExpirados = meusAlugueis.filter(
    (item) => item.status === "EXPIRADA",
  );
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-300 font-sans antialiased pb-10 relative overflow-x-hidden">
      {toast.visivel && (
        <div
          className={`fixed top-6 right-6 z-[150] px-5 py-4 rounded-xl shadow-2xl border transition-all duration-300 animate-fade-in flex items-center gap-3 max-w-sm ${
            toast.tipo === "sucesso"
            ...
            <form onSubmit={salvarEdicaoJogo} className="flex flex-col gap-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`${adminInputClass} resize-none h-24`}
                  required
                />
              </div>
              ...
              <div className="flex gap-3 mt-4 pt-4 border-t border-zinc-800">
                <button
                  ...
                  className="flex-1 py-3 rounded-xl font-bold text-xs bg-zinc-800 text-zinc-300 hover:bg-zinc-700 hover:text-white uppercase tracking-wide transition-colors"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="flex-1 py-3 rounded-xl font-bold text-xs text-white uppercase tracking-wide shadow-lg transition-all bg-blue-600 hover:bg-blue-500 shadow-blue-600/20"
                >
                  Salvar Alterações
                </button>
              </div>
          ...
          <div className="bg-gradient-to-br from-purple-900/20 to-zinc-900 border border-purple-500/30 rounded-3xl p-6 md:p-8 max-w-2xl w-full shadow-2xl shadow-purple-500/10 overflow-y-auto max-h-[90vh] custom-scrollbar">
            ...
            >
              ...
              <div className="flex gap-3 mt-4 pt-4 border-t border-zinc-800">
                ...
                <button
                  type="submit"
                  className="flex-1 py-3 rounded-xl font-bold text-xs text-white uppercase tracking-wide shadow-lg transition-all bg-purple-600 hover:bg-purple-500 shadow-purple-600/20"
                >
                  Salvar Alterações
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-zinc-900 border border-zinc-700 rounded-3xl p-8 max-w-sm w-full shadow-2xl">
            ...
            <p className="text-sm text-zinc-400 mb-6 leading-relaxed">
              Você está prestes a{" "}
              {modalConfirmacao.tipo === "aluguel" ? "alugar" : "reservar"}{" "}
              <strong className="text-white">
                {modalConfirmacao.jogoTitulo}
              </strong>{" "}
              por{" "}
                ...
                <>
                  <div className="bg-zinc-950 rounded-2xl p-5 mb-8 border border-zinc-800 shadow-inner">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span
                        className={`text-base font-black ${temSaldo ? "text-emerald-400" : "text-rose-500 animate-pulse"}`}
                      >
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      disabled={!temSaldo}
                      className={`flex-1 py-3 rounded-xl font-bold text-xs text-white uppercase tracking-wide shadow-lg transition-all ${!temSaldo ? "opacity-50 cursor-not-allowed bg-zinc-600" : modalConfirmacao.tipo === "aluguel" ? "bg-blue-600 hover:bg-blue-500 shadow-blue-600/20" : "bg-amber-600 hover:bg-amber-500 shadow-amber-600/20"}`}
                    >
      ...
      {modalDescricao && (
        ...
        >
          <div
            className="bg-zinc-900 border border-zinc-700 rounded-3xl p-8 max-w-lg w-full shadow-2xl relative"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setModalDescricao(null)}
              className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-white transition-colors"
            >
              ✕
            </button>
            <h3 className="text-xl font-black text-white mb-4 tracking-tight pr-8 leading-tight">
            ...
            <div className="mt-6 pt-4 border-t border-zinc-800">
              <button
                onClick={() => setModalDescricao(null)}
                className="w-full py-3.5 rounded-xl font-bold text-xs bg-zinc-800 text-white hover:bg-zinc-700 uppercase tracking-wide transition-colors"
              >
                Fechar
              </button>
            </div>
      ...
      !modoEsqueciSenha ? (
        <div
          className="flex justify-center items-center min-h-screen p-4 transition-all duration-1000 ease-in-out"
          style={{
        ...
        >
          ...
          <div className="relative z-10 bg-zinc-900 p-8 md:p-10 rounded-3xl border border-zinc-800 w-full max-w-md shadow-2xl animate-fade-in">
            <h2 className="text-3xl font-black text-center text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-emerald-400 mb-8 tracking-tighter">
              BORA JOGAR!
            </h2>
            <span className={subtitleCyberClass}>
            ...
            >
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="text-[10px] font-bold text-zinc-400 hover:text-blue-400 transition-colors uppercase tracking-wider"
                >
                  Esqueceu a senha?
                </button>
              </div>
              ...
              <button
                type="submit"
                className="w-full py-4 bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm uppercase tracking-wide rounded-xl transition-all shadow-lg shadow-blue-500/30"
              >
                Entrar na Loja
              </button>
              ...
              <div className="pt-6 text-center border-t border-zinc-800 mt-8">
                <p className="text-sm text-zinc-400 leading-relaxed">
                  ...
                  <button
                    ...
                    className="mt-3 text-sm font-black text-emerald-400 hover:text-emerald-300 uppercase tracking-wider transition-colors"
                  >
          ...
          <div className="relative z-10 bg-zinc-900 p-8 md:p-10 rounded-3xl border border-zinc-800 w-full max-w-md shadow-2xl animate-fade-in">
            ...
            >
              ...
              <input
                ...
                onChange={(e) =>
                  setCadCodigoConvite(e.target.value.toUpperCase())
                }
                className={`${inputClass} border-purple-500/50 bg-purple-950/20 text-purple-100 placeholder-purple-400/50 uppercase`}
              />
              ...
              <div className="bg-zinc-950/50 border border-zinc-800 p-4 rounded-xl mt-4 shadow-inner">
                <p className="text-[11px] text-zinc-400 leading-relaxed text-center">
                  Ao clicar em "Finalizar Cadastro", você confirma que é maior
                  de idade e declara que leu, compreendeu e concorda
                  integralmente com os nossos{" "}
                  <button
                    type="button"
                    onClick={() => {
                      setAbaAtual("termos");
                    ...
                    className="text-emerald-400 font-bold hover:text-emerald-300 transition-colors underline decoration-emerald-500/50 underline-offset-2 hover:decoration-emerald-400"
                  >
                    Termos de Uso
                  </button>{" "}
                  e{" "}
              ...
              <button
                type="submit"
                className="w-full py-4 mt-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm uppercase tracking-wide rounded-xl transition-all shadow-lg shadow-emerald-500/30"
              >
                Finalizar Cadastro
              </button>
              ...
              <div className="pt-6 text-center border-t border-zinc-800 mt-6">
                <p className="text-sm text-zinc-400 leading-relaxed">
                  ...
                  <button
                    ...
                    className="mt-3 text-sm font-black text-blue-400 hover:text-blue-300 uppercase tracking-wider transition-colors"
                  >
                    Faça Login aqui
                  </button>
                </p>
          ...
          <div className="relative z-10 bg-zinc-900 p-8 md:p-10 rounded-3xl border border-zinc-800 w-full max-w-md shadow-2xl animate-fade-in">
            ...
            >
              <p className="text-sm text-zinc-400 text-center mb-6 leading-relaxed">
                Digite seu e-mail de cadastro. Se ele existir, enviaremos uma
                senha temporária em instantes.
              </p>
              <input
                type="email"
                placeholder="Seu E-mail"
              ...
              <button
                type="submit"
                className="w-full py-4 bg-amber-600 hover:bg-amber-500 text-white font-bold text-sm uppercase tracking-wide rounded-xl transition-all shadow-lg shadow-amber-500/30"
              >
                Recuperar Senha
              </button>
              <div className="pt-6 text-center border-t border-zinc-800 mt-8">
                <button
                  ...
                  className="text-sm font-bold text-zinc-400 hover:text-white uppercase tracking-wider transition-colors"
                >
                  Voltar para o Login
                </button>
              </div>
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8">
              <div className="flex items-center justify-between h-16">
                <div className="flex items-center gap-8">
                  <span
                    className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-emerald-400 tracking-tighter cursor-pointer"
                  ...
                  <div className="hidden md:flex space-x-2">
                    <button
                      onClick={() => setAbaAtual("vitrine")}
                      className={`${navBtnClass} ${abaAtual === "vitrine" ? "bg-blue-600 text-white shadow-md shadow-blue-600/20" : "text-zinc-400 hover:text-white hover:bg-zinc-800"}`}
                    >
                      🎮 Loja
                    </button>
                    <button
                      onClick={() => setAbaAtual("faq")}
                      className={`${navBtnClass} ${abaAtual === "faq" ? "bg-purple-600 text-white shadow-md shadow-purple-600/20" : "text-zinc-400 hover:text-white hover:bg-zinc-800"}`}
                    >
                      📖 Como Funciona
                    </button>
                  </div>
                ...
                <div className="flex items-center gap-4">
                  {!usuarioLogado ? (
                    <button
                      ...
                      className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2 rounded-xl transition-colors text-xs font-bold uppercase tracking-wider"
                    >
                      Entrar
                    </button>
                  ) : (
                    <>
                      <div className="hidden md:flex items-center space-x-2 mr-2 border-r border-zinc-800 pr-4">
                        <button
                          ...
                          className={`${navBtnClass} ${abaAtual === "dashboard" ? "bg-emerald-600 text-white shadow-md shadow-emerald-600/20" : "text-zinc-400 hover:text-white hover:bg-zinc-800"}`}
                        >
                          🔑 Meus Acessos
                        </button>
                        ...
                        {usuarioLogado.is_admin && (
                          <button
                            ...
                            className={`${navBtnClass} ${abaAtual === "admin" ? "bg-rose-600 text-white shadow-md shadow-rose-600/20" : "text-zinc-400 hover:text-white hover:bg-zinc-800"}`}
                          >
                            ⚙️ Painel Admin
                          </button>
                        )}
                      ...
                      <div className="hidden md:flex bg-zinc-950 border border-zinc-800 px-4 py-2 rounded-xl items-center gap-3 shadow-inner">
                        ...
                        <span
                          className={`text-sm font-black ${usuarioLogado.saldo < 0 ? "text-rose-500 animate-pulse" : "text-emerald-400"}`}
                        >
                      ...
                      <button
                        ...
                        className="hidden md:flex relative items-center justify-center w-10 h-10 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-orange-500/50 hover:bg-orange-500/10 transition-all group"
                      >
                        <span
                      ...
                      <button
                        onClick={sair}
                        className="hidden md:block bg-zinc-800 hover:bg-rose-600 hover:text-white text-zinc-300 px-4 py-2 rounded-xl transition-colors text-xs font-bold uppercase tracking-wider"
                      >
                        Sair
                      </button>
                      ...
                      <button
                        ...
                        className="md:hidden text-zinc-300 hover:text-white p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                      >
                        <svg
                          className="w-7 h-7"
                          fill="none"
              ...
              <div className="md:hidden bg-zinc-900 border-b border-zinc-800 absolute w-full left-0 top-16 shadow-2xl animate-fade-in flex flex-col">
                <div className="flex items-center justify-between p-5 border-b border-zinc-800/50 bg-zinc-950/50">
                  <span className="text-sm text-zinc-400">
                    Olá,{" "}
                    <strong className="text-white truncate max-w-[120px] inline-block align-bottom">
                      {usuarioLogado.nome}
                    </strong>
                  </span>
                ...
                <div className="flex flex-col p-5 gap-3">
                  <button
                    ...
                    className={`p-4 rounded-xl text-left text-sm font-bold uppercase tracking-wider transition-all shadow-md ${abaAtual === "vitrine" ? "bg-blue-600 text-white shadow-blue-600/20" : "bg-zinc-800 text-zinc-300 hover:bg-zinc-700 hover:text-white"}`}
                  >
                    🎮 Loja
                  </button>
                  <button
                    ...
                    className={`p-4 rounded-xl text-left text-sm font-bold uppercase tracking-wider transition-all shadow-md ${abaAtual === "dashboard" ? "bg-emerald-600 text-white shadow-emerald-600/20" : "bg-zinc-800 text-zinc-300 hover:bg-zinc-700 hover:text-white"}`}
                  >
                    🔑 Meus Acessos
                  </button>
                  <button
                    ...
                    className={`p-4 rounded-xl text-left text-sm font-bold uppercase tracking-wider transition-all shadow-md ${abaAtual === "faq" ? "bg-purple-600 text-white shadow-purple-600/20" : "bg-zinc-800 text-zinc-300 hover:bg-zinc-700 hover:text-white"}`}
                  >
                    📖 Como Funciona
                  </button>
                  ...
                  {usuarioLogado.is_admin && (
                    <button
                      ...
                      className={`p-4 rounded-xl text-left text-sm font-bold uppercase tracking-wider transition-all shadow-md ${abaAtual === "admin" ? "bg-rose-600 text-white shadow-rose-600/20" : "bg-zinc-800 text-zinc-300 hover:bg-zinc-700 hover:text-white"}`}
                    >
                      ⚙️ Painel Admin
                    </button>
                  )}
                  ...
                  <button
                    ...
                    className="p-4 mt-4 rounded-xl text-center text-sm font-bold uppercase tracking-wider transition-all bg-rose-900/30 text-rose-400 border border-rose-500/30 hover:bg-rose-600 hover:text-white shadow-lg"
                  >
                    Sair da Conta
                  </button>
                </div>
          ...
          <main className="max-w-7xl mx-auto px-4 pb-12 pt-24 md:px-8 md:pt-28">
            {abaAtual === "vitrine" && (
              <div className="animate-fade-in">
                <div
                  className="relative rounded-3xl p-8 md:p-14 mb-10 border border-zinc-800 overflow-hidden shadow-2xl flex items-center min-h-[360px] transition-all duration-700"
                  style={{
                    backgroundImage: `url('${currentBanner}')`,
                    backgroundSize: "cover",
                    backgroundPosition: "center",
                ...
                >
                  ...
                  <div className="relative z-10 w-full">
                    ...
                    <p className="text-sm md:text-base text-zinc-300 max-w-xl mb-8 leading-relaxed font-medium">
                      Alugue os maiores lançamentos e os melhores exclusivos.
                      Receba seu acesso instantaneamente e comece a jogar sem
                      sair de casa.
                    </p>
                    ...
                    <div className="relative max-w-xl group">
                      ...
                      <input
                        type="text"
                        placeholder="Qual jogo você quer jogar hoje?"
                        value={termoBusca}
                        onChange={lidarComBusca}
                        className="w-full py-4 pl-16 pr-6 bg-zinc-900/60 backdrop-blur-xl border border-zinc-700/50 rounded-2xl text-sm font-medium text-white placeholder-zinc-400 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20 transition-all shadow-xl"
                      />
                    </div>
                    ...
                    <div className="flex flex-wrap gap-4 mt-6 max-w-xl">
                      <div className="flex bg-zinc-900/80 rounded-xl p-1.5 backdrop-blur-md border border-zinc-700/50 shadow-lg overflow-x-auto scrollbar-hide">
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-all whitespace-nowrap ${filtroPlataforma === "TODAS" ? "bg-blue-600 text-white shadow" : "text-zinc-400 hover:text-white hover:bg-zinc-800"}`}
                        >
                          Todas
                        </button>
                        <button
                          onClick={() => lidarComFiltroPlataforma("PS5")}
                          className={`px-4 py-2 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-all whitespace-nowrap ${filtroPlataforma === "PS5" ? "bg-blue-600 text-white shadow" : "text-zinc-400 hover:text-white hover:bg-zinc-800"}`}
                        >
                          PS5
                        </button>
                        <button
                          onClick={() => lidarComFiltroPlataforma("PS4/PS5")}
                          className={`px-4 py-2 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-all whitespace-nowrap ${filtroPlataforma === "PS4/PS5" ? "bg-blue-600 text-white shadow" : "text-zinc-400 hover:text-white hover:bg-zinc-800"}`}
                        >
                          PS4/PS5
                        </button>
                      </div>
                      ...
                      <div className="flex bg-zinc-900/80 rounded-xl p-1.5 backdrop-blur-md border border-zinc-700/50 shadow-lg">
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-all ${filtroDisponibilidade === "TODOS" ? "bg-emerald-600 text-white shadow" : "text-zinc-400 hover:text-white hover:bg-zinc-800"}`}
                        >
                          Status: Todos
                        </button>
                        <button
                          onClick={() => lidarComFiltroDisp("DISPONIVEL")}
                          className={`px-4 py-2 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-all ${filtroDisponibilidade === "DISPONIVEL" ? "bg-emerald-600 text-white shadow" : "text-zinc-400 hover:text-white hover:bg-zinc-800"}`}
                        >
                          Só Disponíveis
                        </button>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-5 md:gap-0 md:divide-x md:divide-zinc-800">
                      ...
                      <div className="flex flex-col md:items-center md:text-center px-4 md:pr-6 md:pl-0">
                        ...
                        <p className="text-zinc-300 text-xs leading-relaxed">
                          ...
                          <strong className="text-white">
                            vários consoles ao mesmo tempo
                          </strong>
                          .
                        </p>
                      </div>
                      ...
                      <div className="flex flex-col md:items-center md:text-center px-4 md:px-6 mt-2 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-zinc-800">
                        ...
                        <p className="text-zinc-300 text-xs leading-relaxed">
                          A conta é{" "}
                          <strong className="text-white">SÓ SUA</strong> durante
                          o aluguel. Jogue online, ganhe troféus, sem cair.
                        </p>
                      </div>
                      ...
                      <div className="flex flex-col md:items-center md:text-center px-4 md:pl-6 md:pr-0 mt-2 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-zinc-800">
                        ...
                        <p className="text-zinc-300 text-xs leading-relaxed">
                          Pague{" "}
                          <strong className="text-white">apenas R$ 30</strong>,
                          zere lançamentos e parta para o próximo jogo.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {carregandoJogos && (
                  <div className="mb-8 py-20 flex flex-col items-center justify-center text-center animate-fade-in bg-zinc-900/50 rounded-3xl border border-zinc-800 shadow-xl">
                    ...
                    <p className="text-sm text-zinc-400 font-medium max-w-md leading-relaxed">
                      Nosso sistema está estabelecendo uma conexão segura com o
                      banco de dados. Isso pode levar até 50 segundos. Segura
                      aí!
                    </p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                  {jogosDaPagina.map((jogo) => {
                    ...
                    const isVeterano = usuarioLogado
                      ...
                      : false;
                    const bloqueadoParaUsuario =
                      ehJogoVIP && usuarioLogado && !isVeterano;
                    ...
                    const diasFilaEsperaMs =
                      (jogo.fila_dias_espera || 0) * 24 * 60 * 60 * 1000;
                    const dataFinalExata = new Date(
                      dataVagaGlobal.getTime() + diasFilaEsperaMs,
                    );

                    const dataVagaGlobalStr =
                      dataFinalExata.toLocaleDateString("pt-BR");

                    return (
                      <div
                        key={jogo.id}
                        className={`bg-zinc-900 rounded-3xl border ${bloqueadoParaUsuario ? "border-zinc-800/50 opacity-90 grayscale-[20%]" : "border-zinc-800"} flex flex-col shadow-xl hover:-translate-y-2 hover:border-blue-500/50 transition-all duration-300 group overflow-hidden`}
                      >
                          ...
                          <div className="absolute top-4 left-4 right-4 flex justify-between items-start pointer-events-none">
                            ...
                            <div className="flex flex-col items-end">
                              {jogo.estoque > 0 && !isEmBreve ? (
                                <span className="bg-emerald-500/90 backdrop-blur-md text-white text-[10px] uppercase tracking-wider font-black px-3 py-1.5 rounded-lg flex items-center gap-1.5 shadow-lg border border-emerald-400/50 [text-shadow:1px_1px_0px_black,-1px_-1px_0px_black,1px_-1px_0px_black,-1px_1px_0px_black]">
                                  <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse"></span>
                                  DISPONÍVEL
                                </span>
                              ) : isEmBreve ? (
                                <span className="bg-purple-600/90 backdrop-blur-md text-white text-[10px] uppercase tracking-widest font-black px-3 py-1.5 rounded-lg shadow-lg border border-purple-500/50 [text-shadow:1px_1px_0px_black,-1px_-1px_0px_black,1px_-1px_0px_black,-1px_1px_0px_black]">
                                  LANÇAMENTO{" "}
                                  {dataFormatada && `(${dataFormatada})`}
                                </span>
                              ) : (
                                <span className="bg-rose-600/90 backdrop-blur-md text-white text-[10px] uppercase tracking-wider font-black px-3 py-1.5 rounded-lg shadow-lg border border-rose-500/50 [text-shadow:1px_1px_0px_black,-1px_-1px_0px_black,1px_-1px_0px_black,-1px_1px_0px_black]">
                                  ALUGADO
                                </span>
                              )}
                            </div>
                        ...
                        <div className="p-6 flex flex-col flex-1">
                          <h3 className="text-lg font-black text-white mb-2 tracking-tight group-hover:text-blue-400 transition-colors leading-tight">
                            {tituloLimpo}
                          </h3>
                          <div className="mb-6">
                            <p
                              className="text-xs text-zinc-400 line-clamp-4 leading-relaxed font-medium"
                          ...
                          <div className="mt-auto">
                            ...
                            <div className="flex gap-3 w-full mt-3">
                              {minhaReservaAtiva ? (
                                ...
                                >
                                  <span className="text-[10px] uppercase tracking-widest font-bold text-zinc-500 mb-1 group-hover/lock:text-rose-400 transition-colors">
                                    Acesso Restrito
                                  </span>
                                ...
                                <>
                                  ...
                                  {jogo.preco_aluguel_14 > 0 && (
                                    ...
                                    >
                                      <span
                                        className={`absolute -top-3 right-2 text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider shadow-lg border bg-zinc-950 z-10 ${jogo.estoque > 0 && !isEmBreve ? "text-cyan-400 border-cyan-500/50" : "text-orange-400 border-orange-500/50"}`}
                                      >
                                        PROMO
                                      </span>
                                      <span className="text-[10px] uppercase tracking-wider font-black text-white/90 group-hover:text-white transition-colors [text-shadow:1px_1px_0px_black,-1px_-1px_0px_black,1px_-1px_0px_black,-1px_1px_0px_black]">
              ...
              <div className="animate-fade-in max-w-5xl mx-auto space-y-8">
                ...
                <div className="bg-gradient-to-r from-purple-900/20 via-zinc-900 to-zinc-900 border border-purple-500/30 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-6 shadow-2xl relative overflow-hidden">
                  ...
                  <div className="flex-1 text-center md:text-left z-10">
                    ...
                    <p className="text-sm font-mono-tech text-zinc-400 mb-4">
                      {usuarioLogado.email}
                    </p>
                    ...
                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                      ...
                      <span
                        className={`${meuRank.cor} px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 shadow-[0_0_10px_rgba(255,255,255,0.05)]`}
                      >
                ...
                {notificacoes.map((notif) => (
                  <div
                    key={notif.id}
                    className="bg-orange-950/30 border border-orange-500/40 rounded-3xl p-6 md:p-8 shadow-[0_0_20px_rgba(249,115,22,0.1)] flex flex-col gap-4 animate-fade-in relative overflow-hidden mb-4"
                  >
                    <div className="absolute top-0 left-0 w-1 h-full bg-orange-500"></div>
                    <div className="flex items-start gap-4">
                      <span className="text-3xl animate-bounce">⚠️</span>
                      <div>
                        <h3 className="text-orange-400 font-black text-lg tracking-tight uppercase mb-1">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 mt-2 pl-0 sm:pl-12">
                      <button
                        ...
                        className="flex-1 bg-orange-600 hover:bg-orange-500 text-white font-bold uppercase tracking-wider py-3 px-6 rounded-xl text-xs transition-colors shadow-lg shadow-orange-600/20"
                      >
                        👍 Entendi, manter reserva
                      </button>
                      <button
                        onClick={() =>
                          cancelarReservaComEstorno(notif.reserva_id, notif.id)
                        }
                        className="flex-1 bg-zinc-900 hover:bg-zinc-800 text-zinc-300 font-bold uppercase tracking-wider py-3 px-6 rounded-xl border border-zinc-700 text-xs transition-colors"
                      >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                  <section className="bg-gradient-to-br from-cyan-900/20 to-zinc-900 p-6 md:p-8 rounded-3xl border border-cyan-500/30 shadow-2xl shadow-cyan-500/10 flex flex-col h-auto lg:h-[540px] relative overflow-hidden hover:-translate-y-1 transition-transform duration-300">
                    <div className="absolute -right-8 -top-8 text-9xl opacity-5 pointer-events-none">
                      💸
                    </div>
                    <h3 className="text-lg font-black text-cyan-400 mb-2 tracking-tight flex items-center gap-2">
                      💰 Adicionar Saldo via PIX
                    </h3>
                    <p className="text-xs text-zinc-400 mb-4 leading-relaxed">
                      Recarregue sua carteira instantaneamente para alugar jogos
                      sem filas.
                    </p>
                    ...
                    {pixPendente ? (
                      <div className="flex flex-col items-center justify-center animate-fade-in z-10 bg-zinc-950 p-6 rounded-2xl border border-cyan-500/30 shadow-inner mt-auto mb-auto">
                        <img
                          ...
                          alt="QR Code PIX"
                          className="w-40 h-40 rounded-xl border border-zinc-800 p-2 mb-4 bg-white shadow-lg"
                        />
                        <p className="text-[11px] font-medium text-zinc-400 mb-4 text-center leading-relaxed">
                          Escaneie o QR Code ou copie o código abaixo.{" "}
                          <strong className="text-cyan-400 block mt-1 text-xs font-bold animate-pulse">
                          ...
                          <button
                            onClick={() => {
                              navigator.clipboard.writeText(
                                pixPendente.copia_cola,
                              );
                              mostrarToast("PIX Copiado!", "sucesso");
                            }}
                            className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-white font-bold py-3 rounded-xl text-[10px] uppercase tracking-wide transition-colors border border-zinc-700"
                          >
                            📋 Copiar Código
                          </button>
                          <button
                            onClick={() => setPixPendente(null)}
                            className="bg-rose-900/30 hover:bg-rose-900/80 text-rose-400 font-bold px-4 rounded-xl text-[10px] uppercase tracking-wide transition-colors border border-rose-500/30"
                          >
                            Cancelar
                          </button>
                        </div>
                      ...
                      >
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full pl-12 pr-4 py-3 bg-zinc-950 border border-zinc-800 text-white rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none font-black text-base transition-all"
                              required
                            />
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            maxLength="14"
                            className="w-full px-4 py-3 bg-zinc-950 border border-zinc-800 text-white font-bold rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none text-sm placeholder-zinc-600 transition-all"
                            required
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={cupomRecarga}
                            onChange={(e) =>
                              setCupomRecarga(e.target.value.toUpperCase())
                            }
                            className="w-full px-4 py-3 bg-zinc-950 border border-zinc-800 text-white font-bold rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none uppercase placeholder:normal-case placeholder-zinc-600 text-sm transition-all"
                          />
                        </div>

                        <button
                          type="submit"
                          className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-black uppercase tracking-wider py-3.5 px-6 rounded-xl transition-all shadow-lg shadow-cyan-600/20 mt-1 flex items-center justify-center gap-2 text-xs"
                        >
                          <span>Gerar PIX</span>{" "}
                    ...
                    {extrato.length === 0 ? (
                      ...
                      <div className="space-y-4 overflow-y-auto pr-3 scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-transparent flex-1">
                        {extrato.map((t, i) => (
                          <div
                            key={i}
                            className="flex justify-between items-center bg-zinc-950/50 p-5 rounded-2xl border border-zinc-800/50 hover:border-zinc-700 transition-colors"
                          >
                            <div className="min-w-0 pr-4">
                              <p
                                className="text-xs md:text-sm font-bold text-zinc-200 truncate"
                                title={t.descricao}
                              >
                                {t.descricao}
                              </p>
                            ...
                            <span
                              className={`text-base md:text-lg font-black tracking-tight shrink-0 ${t.tipo === "ENTRADA" ? "text-emerald-400" : "text-rose-400"}`}
                            >
                              {t.tipo === "ENTRADA" ? "+" : "-"} R${" "}
                ...
                <details className="group bg-gradient-to-r from-rose-900/20 to-zinc-900 rounded-3xl border border-rose-500/30 shadow-2xl shadow-rose-500/10 [&_summary::-webkit-details-marker]:hidden overflow-hidden">
                  <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer text-white font-bold select-none relative hover:bg-rose-900/10 transition-colors">
                    <span className="flex items-center gap-3 text-lg font-black text-rose-400 tracking-tight relative z-10">
                      🔐 Segurança da Conta
                    </span>
                    <span className="transition duration-300 group-open:-rotate-180 text-rose-500 relative z-10 text-lg">
                      ▼
                    </span>
                    <div className="absolute -right-8 -top-8 text-9xl opacity-5 pointer-events-none transition-transform duration-500 group-open:scale-110">
                      🔐
                    </div>
                  </summary>
                  ...
                  <div className="px-6 md:px-8 pb-6 md:pb-8 border-t border-rose-500/20 pt-8 relative z-10 animate-fade-in">
                    <p className="text-xs text-zinc-400 mb-8 max-w-2xl leading-relaxed">
                      ...
                      ou troque a senha temporária que enviamos por e-mail.
                    </p>
                    <form
                      onSubmit={alterarMinhaSenha}
                      className="flex flex-col gap-5 items-end max-w-4xl"
                    >
                        ...
                        <div className="w-full relative">
                          ...
                          <div className="relative">
                            <input
                              ...
                              onChange={(e) =>
                                setMudarSenhaAtual(e.target.value)
                              }
                        ...
                        <div className="w-full relative">
                          ...
                          <div className="relative">
                            <input
                              ...
                              onChange={(e) =>
                                setMudarSenhaNova(e.target.value)
                              }
                              className={`${adminInputClass} pr-12`}
                              required
                            />
                        ...
                        <div className="w-full relative">
                          ...
                          <div className="relative">
                            <input
                              ...
                              onChange={(e) =>
                                setMudarSenhaNovaConfirmacao(e.target.value)
                              }
                              className={`${adminInputClass} pr-12`}
                              required
                            />
                            <button
                              type="button"
                              onClick={() =>
                                setVerSenhaNovaConf(!verSenhaNovaConf)
                              }
                              className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-white transition-colors"
                            >
                              {verSenhaNovaConf ? "🙈" : "👁️"}
                      ...
                      <button
                        type="submit"
                        className="w-full sm:w-auto bg-rose-600 hover:bg-rose-500 text-white font-bold uppercase tracking-wider px-8 py-3.5 rounded-2xl transition-colors border border-rose-500/50 shadow-lg shadow-rose-600/20 whitespace-nowrap text-xs mt-2"
                      >
                        Atualizar Senha
                      </button>
                    </form>
                ...
                {usuarioLogado && usuarioLogado.codigo_indicacao && (
                  <details className="group bg-gradient-to-r from-purple-900/30 to-blue-900/20 rounded-3xl border border-purple-500/30 shadow-2xl shadow-purple-500/10 [&_summary::-webkit-details-marker]:hidden overflow-hidden">
                    <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer select-none relative hover:bg-purple-900/10 transition-colors">
                      <span className="text-lg font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-400 tracking-tight relative z-10">
                        🎁 Indique um Amigo e Ganhe Bônus!
                      </span>
                      <span className="transition duration-300 group-open:-rotate-180 text-purple-400 relative z-10 text-lg">
                        ▼
                      </span>
                      <div className="absolute -right-10 -top-10 text-9xl opacity-5 transition-transform duration-700 group-open:scale-110 pointer-events-none">
                        🎁
                      </div>
                    </summary>
                    ...
                    <div className="px-6 md:px-8 pb-6 md:pb-8 border-t border-purple-500/20 pt-8 animate-fade-in relative z-10">
                      <p className="text-xs text-zinc-300 mb-8 max-w-3xl leading-relaxed">
                        Mande o seu código para um amigo. Quando ele criar uma
                        conta nova e fizer a{" "}
                        <strong className="text-emerald-400">
                          primeira recarga
                        </strong>
                        , nós daremos <strong>10% do valor</strong> da recarga
                        dele de presente para você gastar em jogos!
                      </p>
                      <div className="flex flex-col sm:flex-row gap-5 items-center">
                        <div className="bg-zinc-950 border border-zinc-800 px-8 py-3.5 rounded-2xl flex items-center gap-5 w-full sm:w-auto justify-center shadow-inner">
                          ...
                          <span className="text-2xl font-black text-white tracking-widest select-all">
                        ...
                        <button
                          ...
                          className="bg-purple-600 hover:bg-purple-500 text-white font-bold uppercase tracking-wider px-8 py-4 rounded-2xl text-xs transition-colors shadow-lg shadow-purple-600/20 w-full sm:w-auto"
                        >
                          📋 Copiar Código
                        </button>
                      </div>
                ...
                <details
                  className="group bg-gradient-to-r from-emerald-900/20 to-zinc-900 rounded-3xl border border-emerald-500/30 border-l-4 border-l-emerald-500 shadow-2xl shadow-emerald-500/10 [&_summary::-webkit-details-marker]:hidden overflow-hidden"
                  open
                >
                  <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer select-none hover:bg-emerald-900/10 transition-colors">
                    <span className="flex items-center gap-3 text-lg font-black text-emerald-400 tracking-tight">
                      🔑 Chaves de Acesso Ativas
                    </span>
                    <span className="transition duration-300 group-open:-rotate-180 text-emerald-500 text-lg">
                      ▼
                    </span>
                  </summary>
                      ...
                      <div className="mb-8 bg-rose-950/40 border border-rose-500/50 rounded-2xl p-5 flex items-start gap-4 shadow-inner">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-zinc-300 leading-relaxed font-medium">
                            É <strong>obrigatório</strong> desativar o
                            "Compartilhamento de Console" ou "PS4 Principal" na
                            sua conta ANTES do tempo de aluguel expirar. O
                            descumprimento gera uma multa automática e deixa seu
                            saldo negativo.
                          </p>
                        </div>
                      </div>
                    ...
                    {alugueisAtivos.length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 gap-6">
                        {alugueisAtivos.map((item) => (
                          <div
                            key={item.locacao_id}
                            className="bg-zinc-950/60 p-6 md:p-8 rounded-3xl border border-emerald-500/30 shadow-xl flex flex-col gap-6 hover:border-emerald-400/50 transition-colors"
                          >
                            ...
                            <div className="flex flex-col gap-3 bg-black/50 p-5 rounded-2xl border border-zinc-800/80 shadow-inner">
                              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                                <span className="text-zinc-500 text-[10px] uppercase font-bold tracking-wider w-14">
                                  Login
                                </span>
                                <span className="text-emerald-400 font-bold text-sm md:text-base select-all break-all tracking-wide">
                                  {item.email_login}
                                </span>
                              </div>
                              ...
                              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                                ...
                                <span className="text-zinc-200 font-mono font-bold text-sm md:text-base bg-zinc-900 px-3 py-1 rounded-lg select-all border border-zinc-700 inline-block w-max tracking-widest">
                                  {item.senha_login}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="w-full">
                              {codigosGerados2FA[item.locacao_id] ? (
                                ...
                                <button
                                  ...
                                  className="w-full bg-emerald-600 hover:bg-emerald-500 text-xs text-white font-bold uppercase tracking-wider py-4 rounded-2xl transition-all shadow-[0_0_15px_rgba(16,185,129,0.4)] hover:shadow-[0_0_25px_rgba(16,185,129,0.6)] flex items-center justify-center gap-3 border border-emerald-400"
                                >
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-2">
                              ...
                              {(() => {
                                const jogoDetalhes = jogos.find(
                                  (j) => j.titulo === item.jogo,
                                );
                                const temFila =
                                  jogoDetalhes && jogoDetalhes.tamanho_fila > 0;
                                if (temFila) {
                                  return (
                                    <button
                                      onClick={() =>
                                      ...
                                      className="w-full sm:w-auto bg-emerald-900/40 hover:bg-emerald-600 text-emerald-400 hover:text-white font-bold uppercase tracking-wider px-8 py-3.5 rounded-xl text-xs transition-all border border-emerald-500/30 shadow-lg flex items-center justify-center gap-2 animate-fade-in"
                                    >
                            ...
                            <details className="mt-6 group/tut bg-gradient-to-r from-emerald-900/30 to-zinc-900 rounded-2xl border border-emerald-500/40 shadow-[0_0_20px_rgba(16,185,129,0.2)] hover:shadow-[0_0_30px_rgba(16,185,129,0.3)] transition-all duration-300 [&_summary::-webkit-details-marker]:hidden overflow-hidden">
                              <summary className="flex items-center justify-between p-5 cursor-pointer text-emerald-400 font-black text-xs md:text-sm select-none hover:bg-emerald-900/30 transition-colors uppercase tracking-wider">
                                <span className="flex items-center gap-2 animate-pulse-slow">
                                  📖 PASSO A PASSO DE COMO ENTRAR NA CONTA E
                                  JOGAR (PS4/PS5)
                                </span>
                              ...
                              <div className="p-5 md:p-8 border-t border-emerald-500/30 text-xs text-zinc-300 space-y-5 bg-black/60">
                                <p className="text-rose-400 font-black uppercase mb-4 tracking-wider flex items-center gap-3 border-b border-rose-500/30 pb-4 text-sm">
                                  ...
                                  ATENÇÃO: NUNCA ENTRE COMO CONVIDADO!
                                </p>
                                <ol className="list-decimal pl-6 space-y-4 font-medium leading-relaxed">
                                  <li>
                                    ...
                                    (na tela de boas vindas dos usuários).
                                  </li>
                                  <li>
                                  ...
                                  <li>
                                    Insira o E-mail e Senha da conta que estão
                                    disponíveis acima.
                                  </li>
                                  <li>
                                    Quando o console pedir o código (2FA),
                                  ...
                                  <li>
                                    Digite o código 2FA (6 dígitos) rapidamente,
                                    ele fica ativo por 30 segundos.
                                  </li>
                                  <li>
                                  ...
                                  <li>
                                    Para jogar na sua conta pessoal e ganhar os
                                    troféus, é OBRIGATÓRIO habilitar o
                                    compartilhamento:
                                    <ul className="list-disc pl-5 mt-3 text-zinc-400 space-y-3 border-l-2 border-zinc-700 ml-1">
                                      <li>
                                        <strong>No PS5:</strong> Vá em
                                        Configurações &gt; Usuários e contas
                                        &gt; Outros &gt; Compartilhamento do
                                        console... &gt;{" "}
                                        <strong className="text-white">
                                          Habilitar
                                        </strong>
                                      ...
                                      <li>
                                        <strong>No PS4:</strong> Vá em
                                        Configurações &gt; Gerenciamento da
                                        conta &gt;{" "}
                                        <strong className="text-white">
                                      ...
                                      <li className="text-rose-400 font-bold flex items-center gap-3 mt-3 bg-rose-950/30 p-3.5 rounded-xl border border-rose-500/20">
                                        <span className="text-lg">⚠️</span> É
                                        aqui também, que no final do seu
                                        aluguel, você vai DESABILITAR o
                                        compartilhamento.
                                      </li>
                                    </ul>
                                  </li>
                                  <li className="text-emerald-400 font-black mt-6 text-sm bg-emerald-950/40 p-5 rounded-xl border border-emerald-500/40 shadow-inner">
                                    Vá na Biblioteca da conta, coloque o jogo
                                    para baixar, volte para o seu perfil pessoal
                                    (a sua conta oficial) e divirta-se!
                                  </li>
                                </ol>
                              </div>
                            </details>
                ...
                <details className="group bg-gradient-to-r from-amber-900/20 to-zinc-900 rounded-3xl border border-amber-500/30 border-l-4 border-l-amber-500 shadow-2xl shadow-amber-500/10 [&_summary::-webkit-details-marker]:hidden overflow-hidden">
                  <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer select-none hover:bg-amber-900/10 transition-colors">
                    <span className="flex items-center gap-3 text-lg font-black text-amber-400 tracking-tight">
                      ⏳ Minhas Reservas (Fila de Espera)
                    </span>
                    <span className="transition duration-300 group-open:-rotate-180 text-amber-500 text-lg">
                      ▼
                    </span>
                  </summary>
                    ...
                    {minhasReservas.length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 gap-6">
                        {minhasReservas.map((item) => (
                          <div
                            key={item.reserva_id}
                            className="bg-zinc-950/60 p-6 md:p-8 rounded-3xl border border-amber-500/30 shadow-xl flex flex-col gap-5 hover:border-amber-400/50 transition-colors"
                          >
                            ...
                            <div className="flex flex-col gap-3 bg-black/50 p-5 rounded-2xl border border-zinc-800/80 shadow-inner mt-2">
                              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                                <span className="text-zinc-500 text-[10px] uppercase font-bold tracking-wider w-20">
                                  Status
                                </span>
                                <span className="text-amber-400 font-black text-[10px] uppercase tracking-wider bg-amber-400/10 px-3 py-1 rounded-lg border border-amber-500/20 w-max">
                                  Aguardando Fila
                                </span>
                              </div>
                              <div className="w-full h-px bg-zinc-800/80 my-1"></div>
                ...
                {historicoAlugueis.length > 0 && (
                  <details className="group bg-zinc-900/80 rounded-3xl border border-zinc-800 shadow-lg [&_summary::-webkit-details-marker]:hidden overflow-hidden">
                    <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer select-none hover:bg-zinc-800/50 transition-colors">
                      ...
                      <span className="transition duration-300 group-open:-rotate-180 text-zinc-600 text-base">
                        ▼
                      </span>
                    </summary>
                    ...
                    <div className="px-6 md:px-8 pb-6 md:pb-8 border-t border-zinc-800/50 pt-6 flex flex-wrap gap-3 animate-fade-in">
                      {historicoAlugueis.map((item) => (
                        <span
                          key={item.locacao_id}
                          className="bg-zinc-950 border border-zinc-800 text-zinc-400 px-5 py-2 rounded-xl text-[10px] font-bold tracking-wide uppercase"
                        >
                          {item.jogo}{" "}
            ...
            {abaAtual === "faq" && (
              <div className="animate-fade-in max-w-4xl mx-auto py-8">
                <div className="text-center mb-14">
                  <h2 className="text-3xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-500 mb-6 tracking-tight">
                    Central de Ajuda
                  </h2>
                  <p className="text-sm text-zinc-400 font-medium">
                    Tudo o que você precisa saber para alugar e jogar sem dores
                    de cabeça.
                  </p>
                </div>
                ...
                <div className="space-y-5">
                  <details className="group bg-zinc-900 border border-zinc-800 rounded-3xl [&_summary::-webkit-details-marker]:hidden shadow-xl hover:-translate-y-1 transition-transform duration-300">
                    <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer text-white hover:text-purple-400 transition-colors">
                      ...
                      <span className="transition duration-300 group-open:-rotate-180 text-purple-500 text-lg">
                        ▼
                      </span>
                    </summary>
                    <div className="px-6 md:px-8 pb-6 md:pb-8 text-zinc-400 text-xs md:text-sm leading-relaxed border-t border-zinc-800/50 pt-6">
                      É super simples! Você adiciona saldo à sua carteira
                      digital, escolhe o jogo na vitrine, escolhe o número de
                      dias que quer ficar com o jogo (7 ou 14 dias) e clica em{" "}
                      <strong className="text-white">"ALUGAR"</strong>. O valor
                      é descontado e os dados da conta (E-mail e Senha) aparecem
                      imediatamente na sua aba{" "}
                      <strong className="text-emerald-400">
                        🔑 MEUS ACESSOS
                      </strong>
                      . O aluguel dura 7 ou 14 dias corridos a partir do momento
                      da compra.
                    </div>
                  </details>
                  ...
                  <details className="group bg-zinc-900 border border-zinc-800 rounded-3xl [&_summary::-webkit-details-marker]:hidden shadow-xl hover:-translate-y-1 transition-transform duration-300">
                    ...
                    <div className="px-6 md:px-8 pb-6 md:pb-8 text-zinc-400 text-xs md:text-sm leading-relaxed border-t border-zinc-800/50 pt-6">
                      Aqui no <strong>BORA JOGAR!</strong>, a sua fidelidade se
                      transforma em vantagens reais. Toda vez que você conclui
                      um aluguel com sucesso, o seu nível sobe. Quanto maior o
                      seu Rank, mais benefícios você tem.
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 my-6">
                        ...
                        <div className="bg-fuchsia-950/20 border border-fuchsia-900/50 p-3 rounded-xl flex flex-col items-center justify-center gap-1 shadow-[0_0_10px_rgba(217,70,239,0.05)]">
                          <span className="text-xl">👑</span>
                          <span className="text-fuchsia-400 font-bold uppercase tracking-wider text-[10px]">
                            Super VIP
                          </span>
                          <span className="text-zinc-300 font-medium text-[10px]">
                            50+ Locações
                      ...
                      <strong className="text-white block mb-2 text-sm">
                      ...
                      <ul className="list-disc pl-5 space-y-4 font-medium text-zinc-300">
                        <li>
                          ...
                          Para garantir que nossos clientes fiéis sempre tenham
                          prioridade, as reservas de jogos em{" "}
                          <strong>PRÉ-VENDA (DATA FUTURA)</strong> são
                          exclusivas para clientes a partir do Rank{" "}
                          <strong>MEMBRO</strong>. Após o lançamento oficial do
                          jogo, ele fica liberado para todos os clientes,
                          incluindo Novatos!
                        </li>
                        <li>
                          ...
                          Se um jogo estiver alugado e você decidir entrar na
                          fila de espera, o seu Rank entra em ação! Se você
                          tiver um Rank maior que outro cliente que já estava na
                          fila, o sistema automaticamente te coloca{" "}
                          <strong>na frente dele</strong>. O cliente que foi
                          ultrapassado receberá um aviso imediato com a nova
                          data de liberação calculada.
                        </li>
                      </ul>
                    </div>
                  </details>
                  ...
                  <details className="group bg-zinc-900 border border-zinc-800 rounded-3xl [&_summary::-webkit-details-marker]:hidden shadow-xl hover:-translate-y-1 transition-transform duration-300">
                    ...
                    <div className="px-6 md:px-8 pb-6 md:pb-8 text-zinc-400 text-xs md:text-sm leading-relaxed border-t border-zinc-800/50 pt-6">
                      <ol className="list-decimal pl-5 space-y-3 text-zinc-300 font-medium">
                        ...
                        <li>
                          Insira o E-mail e Senha da conta que estão disponíveis
                          acima.
                        </li>
                        <li>
                          Quando o console pedir o código (2FA), clique no botão
                          verde{" "}
                          <strong className="text-emerald-400">
                            "GERAR CÓDIGO DE ACESSO (2FA)"
                          </strong>{" "}
                        ...
                        <li>
                          Digite o código 2FA (6 dígitos) rapidamente, ele fica
                          ativo por 30 segundos.
                        </li>
                        <li>
                          <strong className="text-rose-400">
                            NÃO ATIVE MAIS NADA
                        ...
                        <li>
                          ...
                          <ul className="list-disc pl-5 mt-3 text-zinc-400 space-y-3 border-l-2 border-zinc-700 ml-1">
                            <li>
                              <strong>No PS5:</strong> Vá em Configurações &gt;
                              Usuários e contas &gt; Outros &gt;
                            ...
                            <li className="text-rose-400 font-bold flex items-center gap-3 mt-3 bg-rose-950/30 p-3.5 rounded-xl border border-rose-500/20">
                              <span className="text-lg">⚠️</span> É aqui também,
                              que no final do seu aluguel, você vai DESATIVAR o
                              compartilhamento.
                            </li>
                          </ul>
                        </li>
                        <li className="text-emerald-400 font-black mt-6 text-sm bg-emerald-950/40 p-5 rounded-xl border border-emerald-500/40 shadow-inner">
                          Vá na Biblioteca da conta, coloque o jogo para baixar,
                          volte para o seu perfil pessoal (a sua conta oficial)
                          e divirta-se!
                        </li>
                      </ol>
                    </div>
                  ...
                  <details className="group bg-zinc-900 border border-zinc-800 rounded-3xl [&_summary::-webkit-details-marker]:hidden shadow-xl hover:-translate-y-1 transition-transform duration-300">
                    <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer text-white hover:text-purple-400 transition-colors">
                      <span className="text-base md:text-lg font-bold tracking-tight">
                        🔐 O videogame pediu um código de verificação (2FA). O
                        que eu faço?
                      </span>
                      <span className="transition duration-300 group-open:-rotate-180 text-purple-500 text-lg">
                        ▼
                    ...
                    <div className="px-6 md:px-8 pb-6 md:pb-8 text-zinc-400 text-xs md:text-sm leading-relaxed border-t border-zinc-800/50 pt-6">
                      ...
                      , embaixo das informações da conta (Login e Senha), existe
                      um botão verde chamado{" "}
                      <strong className="text-emerald-400">
                        "GERAR CÓDIGO DE ACESSO (2FA)"
                      </strong>
                      .<br />
                      <br />
                      Basta clicar nele que um código de 6 dígitos vai aparecer
                      na sua tela. Digite esse código rapidamente no seu
                      PlayStation (ele muda a cada 30 segundos). Você não
                      precisa mandar mensagem pro suporte, o sistema gera o
                      código para você na hora!
                    </div>
                  </details>
                  ...
                  <details className="group bg-zinc-900 border border-zinc-800 rounded-3xl [&_summary::-webkit-details-marker]:hidden shadow-xl hover:-translate-y-1 transition-transform duration-300">
                    <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer text-white hover:text-purple-400 transition-colors">
                      <span className="text-base md:text-lg font-bold tracking-tight">
                        🏆 Posso jogar na minha conta pessoal e ganhar os
                        troféus?
                      </span>
                      <span className="transition duration-300 group-open:-rotate-180 text-purple-500 text-lg">
                        ▼
                    ...
                    <div className="px-6 md:px-8 pb-6 md:pb-8 text-zinc-400 text-xs md:text-sm leading-relaxed border-t border-zinc-800/50 pt-6">
                      <strong className="text-white">Sim, com certeza!</strong>{" "}
                      Para isso, logo após fazer o login com a conta alugada no
                      console:
                      ...
                      <div className="mt-4">
                        Depois disso, inicie o download do jogo, troque para a
                        sua conta pessoal (a sua oficial) e jogue normalmente.
                        Os saves e troféus ficarão nela!
                      </div>
                    </div>
                  </details>
                  ...
                  <details className="group bg-zinc-900 border border-zinc-800 rounded-3xl [&_summary::-webkit-details-marker]:hidden shadow-xl hover:-translate-y-1 transition-transform duration-300">
                    ...
                    <div className="px-6 md:px-8 pb-6 md:pb-8 text-zinc-400 text-xs md:text-sm leading-relaxed border-t border-zinc-800/50 pt-6">
                      Não se preocupe, você pode garantir a sua vaga! Clique no
                      botão{" "}
                      <strong className="text-amber-400">"RESERVAR"</strong> (7
                      ou 14 dias). O valor do jogo será investido e você verá
                      uma data de <em>Previsão de Liberação</em> em{" "}
                      <strong className="text-emerald-400">
                        🔑 MEUS ACESSOS
                      </strong>
                      . <br />
                      <br />
                      Nosso sistema inteligente repassa a conta automaticamente
                      para você no exato segundo em que o aluguel do cliente
                      anterior terminar.
                    </div>
                  </details>
                  ...
                  <details className="group bg-zinc-900 border border-zinc-800 rounded-3xl [&_summary::-webkit-details-marker]:hidden shadow-xl hover:-translate-y-1 transition-transform duration-300">
                    <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer text-emerald-400 hover:text-emerald-300 transition-colors">
                      <span className="text-base md:text-lg font-bold tracking-tight">
                        ♻️ Posso devolver um jogo antes do prazo e receber
                        cashback?
                      </span>
                      <span className="transition duration-300 group-open:-rotate-180 text-emerald-500 text-lg">
                        ▼
                    ...
                    <div className="px-6 md:px-8 pb-6 md:pb-8 text-zinc-400 text-xs md:text-sm leading-relaxed border-t border-emerald-800/50 pt-6">
                      ...
                      <strong className="text-white">DEVOLUÇÃO DINÂMICA</strong>
                      ! Essa opção fica ativa automaticamente apenas quando o
                      jogo que você alugou está com{" "}
                      <strong className="text-rose-400">ALTA DEMANDA</strong>{" "}
                      (ou seja, quando existem outras pessoas na fila de espera
                      ...
                      aparecerá ao lado do seu jogo na aba{" "}
                      <strong className="text-emerald-400">
                        🔑 MEUS ACESSOS
                      </strong>
                      . Ao fazer a devolução antecipada para agilizar a fila,
                      você ganha uma recompensa:{" "}
                      <strong className="text-emerald-400">
                        R$ {configSistema.valor_por_dia.toFixed(2)} de cashback
                        por cada 24 horas (1 dia completo)
                      </strong>{" "}
                      que ainda restavam no seu prazo!
                      <br />
                      <br />O valor cai direto na sua carteira digital assim que
                      a nossa equipe verificar que a conta foi devidamente
                      desativada do seu console.
                    </div>
                  </details>
                  ...
                  <details className="group bg-rose-950/20 border border-rose-500/30 rounded-3xl [&_summary::-webkit-details-marker]:hidden shadow-xl hover:-translate-y-1 transition-transform duration-300">
                    <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer text-rose-400 hover:text-rose-300 transition-colors">
                      <span className="text-base md:text-lg font-bold tracking-tight">
                        🚨 O que acontece se eu esquecer de desativar a conta do
                        meu videogame?
                      </span>
                      <span className="transition duration-300 group-open:-rotate-180 text-rose-500 text-lg">
                        ▼
                      </span>
                    </summary>
                    <div className="px-6 md:px-8 pb-6 md:pb-8 text-zinc-400 text-xs md:text-sm leading-relaxed border-t border-rose-800/50 pt-6">
                      Essa é a nossa regra mais rigorosa! Se o seu tempo acabar
                      e você deixar a conta ativada como "Principal" no seu
                      console, isso bloqueia a conta e impede que o próximo
                      cliente da fila jogue. Quando isso acontece, nós
                      solicitamos à Sony que faça a desabilitação. Isso pode
                      causar banimento!
                      <br />
                      <br />
                      ...
                      direto na sua carteira digital. Se você não tiver saldo,
                      sua conta ficará negativada e bloqueada para alugar jogos.
                      Por isso, sempre desabilite a conta como "Principal" no
                      seu console quando seu aluguel acabar.
                      <br />
                      <br />
                      Se tiver alguma dúvida sobre como desabilitar a conta como
                      "Principal" no seu console, consulte o tutorial que fica
                      disponível junto com as informações da conta ou entre em
                      contato conosco e te ajudaremos. É simples e rápido!
                    </div>
                  </details>
                </div>
              </div>
            ...
            {abaAtual === "termos" && (
              <div className="animate-fade-in max-w-4xl mx-auto py-8">
                ...
                <div className="space-y-8 text-zinc-300 text-sm md:text-base leading-relaxed">
                  <section>
                    <h3 className="text-xl font-bold text-blue-400 mb-3 tracking-tight">
                      1. Objeto e Natureza do Serviço
                    </h3>
                    <p>
                      A BORA JOGAR! oferece o serviço de locação de licenças de
                      jogos digitais em contas secundárias para os consoles
                      PlayStation 4 e PlayStation 5. O usuário adquire o direito
                      de acessar a conta fornecida, baixar o jogo e jogá-lo em
                      seu perfil pessoal durante o período contratado (7 ou 14
                      dias).
                    </p>
                  </section>
                  ...
                  <section>
                    ...
                    <ul className="list-disc pl-5 space-y-2 text-zinc-400">
                      <li>
                        É expressamente{" "}
                        <strong className="text-rose-400">PROIBIDO</strong>{" "}
                        entrar na conta fornecida utilizando a opção "Jogar como
                        Convidado".
                      </li>
                      <li>
                        O usuário compromete-se a{" "}
                        <strong className="text-white">NÃO ALTERAR</strong>{" "}
                        nenhum dado da conta fornecida, incluindo (mas não se
                        limitando a): E-mail, Senha, ID Online, Avatar ou
                        configurações de segurança (2FA). Qualquer tentativa de
                        alteração será registrada pelo sistema da Sony e
                        resultará no banimento permanente do usuário em nossa
                        plataforma e acionamento das medidas legais cabíveis.
                      </li>
                      <li>
                        O usuário está autorizado a ativar o "Compartilhamento
                        do Console e Jogo Offline" (PS5) ou "Ativar como PS4
                        Principal" (PS4) para jogar em sua conta pessoal e
                  ...
                  <section>
                    <h3 className="text-xl font-bold text-rose-400 mb-3 tracking-tight">
                      3. Devolução Obrigatória e Multa por Atraso
                    </h3>
                    <p>
                      Ao término do período de locação, o usuário perde o
                      direito de acesso ao jogo. É{" "}
                      <strong>obrigação exclusiva do usuário</strong> acessar as
                      configurações do seu console e{" "}
                      <strong className="text-rose-400">DESATIVAR</strong> o
                      Compartilhamento de Console (PS5) ou a conta como
                      Principal (PS4) antes de excluir o usuário do videogame.
                    </p>
                    <div className="bg-rose-950/20 border border-rose-500/30 p-4 rounded-xl mt-4">
                      ...
                      <p className="text-sm text-zinc-400">
                        A não desativação da conta prende a licença no console
                        do usuário, impedindo que o próximo cliente da fila
                        jogue. Caso isso ocorra, o sistema aplicará uma{" "}
                        <strong className="text-white">
                          Multa Administrativa Automática de R$ 50,00
                        </strong>
                        . O saldo do usuário ficará negativo e a conta suspensa
                        até a regularização do débito e liberação da licença.
                      </p>
                    </div>
                  </section>

                  <section>
                    <h3 className="text-xl font-bold text-emerald-400 mb-3 tracking-tight">
                      4. Sistema de Devolução Dinâmica (Cashback)
                    </h3>
                    <p>
                      Caso o jogo alugado possua fila de espera, o botão
                      "Devolver" ficará ativo no painel do usuário. Ao optar
                      pela devolução antecipada para agilizar a fila, o usuário
                      receberá um reembolso em créditos na carteira digital
                      (Cashback) de acordo com os dias inteiros não utilizados,
                      conforme tabela vigente no site no momento da devolução.
                    </p>
                  </section>
                  ...
                  <section>
                    ...
                    <p>
                      Ao reservar um jogo indisponível, o valor integral é
                      retido da carteira do usuário. O sistema transferirá as
                      credenciais de acesso automaticamente no exato momento em
                      que a locação do usuário anterior for encerrada. O prazo
                      de 7 ou 14 dias só começa a contar a partir do momento em
                      que a conta é liberada no painel.
                    </p>
                  </section>
                  ...
                  <section>
                    ...
                    <p>
                      A BORA JOGAR! não se responsabiliza por banimentos
                      sofridos no console do usuário devido à quebra dos Termos
                      de Serviço da PlayStation Network. Caso o usuário utilize
                      softwares de trapaça (hacks/exploits) ou conduta tóxica
                      que resulte no banimento da nossa conta fornecida, o
                      usuário será cobrado judicialmente pelo valor integral de
                      compra do jogo perdido.
                    </p>
                  </section>
                </div>
              </div>
                ...
                <div className="space-y-8 text-zinc-300 text-sm md:text-base leading-relaxed">
                  <section>
                    <p className="text-zinc-400">
                      A sua privacidade é nossa prioridade. Esta política
                      descreve como a BORA JOGAR! coleta, utiliza e protege os
                      seus dados pessoais ao utilizar nossa plataforma, em total
                  ...
                  <section>
                    ...
                    <p>
                      Para o funcionamento da plataforma e liberação das
                      locações, coletamos as seguintes informações no momento do
                      seu cadastro:
                    </p>
                    <ul className="list-disc pl-5 mt-2 space-y-1 text-zinc-400">
                      ...
                      <li>
                        Senha (armazenada de forma criptografada e inacessível
                        até mesmo para nossos administradores).
                      </li>
                    </ul>
                  </section>
                  ...
                  <section>
                    ...
                    <p>
                      As informações coletadas são estritamente utilizadas para:
                    </p>
                    <ul className="list-disc pl-5 mt-2 space-y-1 text-zinc-400">
                      <li>
                        Criar e gerenciar sua carteira digital dentro da
                        plataforma.
                      </li>
                      <li>Enviar credenciais de acesso aos jogos alugados.</li>
                      <li>
                        Notificar via e-mail ou WhatsApp quando a fila de espera
                        do seu jogo favorito andar.
                      </li>
                      <li>
                        Fornecer suporte técnico e prevenir fraudes ou
                        apropriação indevida das contas da locadora.
                      </li>
                    </ul>
                  </section>
                  ...
                  <section>
                    ...
                    <p>
                      A BORA JOGAR!{" "}
                      <strong className="text-white">NÃO armazena</strong> dados
                      bancários, números de cartão de crédito ou chaves PIX de
                      suas contas pessoais. Todo o processamento financeiro é
                      realizado em ambiente blindado e seguro através da
                      instituição de pagamento parceira oficial (Asaas).
                    </p>
                  </section>
                  ...
                  <section>
                    ...
                    <p>
                      Nós não vendemos, alugamos ou repassamos seus dados
                      pessoais para terceiros ou agências de publicidade sob
                      nenhuma hipótese. Seus dados são mantidos em servidores
                      seguros e acessados apenas pelo sistema automatizado para
                      garantir o seu aluguel.
                    </p>
                  </section>
                  ...
                  <section>
                    ...
                    <p>
                      Você tem o direito de solicitar a exclusão da sua conta e
                      de todos os seus dados de nossos servidores a qualquer
                      momento. A exclusão será processada em até 72 horas, desde
                      que não existam locações ativas, pendências de devolução
              ...
              <div className="animate-fade-in mt-2 max-w-6xl mx-auto">
                ...
                <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                  <div className="bg-gradient-to-br from-emerald-900/40 to-zinc-900 border border-emerald-500/30 p-8 rounded-3xl shadow-xl shadow-emerald-500/10 relative overflow-hidden hover:-translate-y-1 transition-transform duration-300">
                    <div className="absolute -right-4 -top-4 text-8xl opacity-5">
                      💰
                    </div>
                    <h4 className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest mb-3">
                      Faturamento Total
                    </h4>
                    <span className="text-3xl md:text-4xl font-black text-emerald-400 tracking-tighter">
                  ...
                  <div className="bg-gradient-to-br from-amber-900/40 to-zinc-900 border border-amber-500/30 p-8 rounded-3xl shadow-xl shadow-amber-500/10 relative overflow-hidden hover:-translate-y-1 transition-transform duration-300">
                    ...
                    <span className="text-3xl md:text-4xl font-black text-amber-400 tracking-tighter">
                ...
                <div className="flex flex-col gap-8 mb-10">
                  ...
                  <details className="group bg-zinc-900/80 rounded-3xl border border-zinc-800 border-l-4 border-l-orange-500 shadow-2xl shadow-orange-500/10 [&_summary::-webkit-details-marker]:hidden overflow-hidden">
                    <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer hover:bg-orange-900/10 transition-colors select-none relative">
                      <span className="flex items-center gap-3 relative z-10 text-lg font-black text-orange-400 tracking-tight">
                        🖼️ Configurações da Vitrine e Banners
                      </span>
                      <span className="transition duration-300 group-open:-rotate-180 text-orange-500 relative z-10 text-lg">
                        ▼
                      </span>
                    </summary>
                    <div className="px-6 md:px-8 pb-6 md:pb-8 border-t border-zinc-800/50 pt-8">
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-5">
                        <div>
                          ...
                          <p className="text-xs text-zinc-400 mt-1 font-medium">
                            Faixa colorida que aparece abaixo dos banners
                            principais.
                          </p>
                        </div>
                        <button
                          onClick={toggleAnuncio}
                          className={`px-6 py-3 rounded-xl font-bold text-xs uppercase tracking-wider transition-all shadow-lg w-full sm:w-auto ${configSistema.anuncio_ativo ? "bg-orange-600 text-white shadow-orange-600/20" : "bg-zinc-800 text-zinc-400 hover:text-white border border-zinc-700"}`}
                        >
                          {configSistema.anuncio_ativo
                            ? "✅ FAIXA LIGADA"
                            : "❌ FAIXA DESLIGADA"}
                      ...
                      <textarea
                        ...
                        className={`${adminInputClass} resize-none h-16 bg-zinc-950 border-zinc-700 focus:ring-orange-500 text-sm`}
                      />
                      ...
                      <div className="mt-8 border-t border-zinc-800/50 pt-6">
                        ...
                        <p className="text-xs text-zinc-400 mt-1 mb-4 font-medium">
                          Cole as URLs das imagens que irão ficar trocando no
                          topo do site.{" "}
                          <strong className="text-emerald-400">
                        ...
                        <textarea
                          ...
                          className={`${adminInputClass} resize-none h-24 bg-zinc-950 border-zinc-700 focus:ring-orange-500 text-sm`}
                        />
                      </div>
                      ...
                      <div className="flex justify-end mt-6">
                        <button
                          ...
                          className="bg-blue-600 hover:bg-blue-500 text-white font-bold uppercase tracking-wider px-8 py-4 rounded-xl shadow-lg shadow-blue-600/20 transition-colors text-xs"
                        >
                  ...
                  <details className="group bg-gradient-to-r from-fuchsia-900/20 to-zinc-900 rounded-3xl border border-fuchsia-500/30 shadow-2xl shadow-fuchsia-500/10 border-l-4 border-l-fuchsia-500 [&_summary::-webkit-details-marker]:hidden overflow-hidden">
                    <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer hover:bg-fuchsia-900/10 transition-colors select-none relative">
                      <span className="flex items-center gap-3 relative z-10 text-lg font-black text-fuchsia-400 tracking-tight">
                        📊 Gestão da Enquete
                      </span>
                      <span className="transition duration-300 group-open:-rotate-180 text-fuchsia-500 relative z-10 text-lg">
                        ▼
                      </span>
                    </summary>
                      ...
                      <div className="flex flex-col lg:flex-row gap-8">
                        ...
                        >
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setNovaOpcaoEnqueteTitulo(e.target.value)
                            }
                            className={adminInputClass}
                            required
                          />
                          <input
                            ...
                            onChange={(e) =>
                              setNovaOpcaoEnqueteImagem(e.target.value)
                            }
                            className={adminInputClass}
                            required
                          />
                          <button
                            type="submit"
                            className="py-3.5 bg-fuchsia-600 hover:bg-fuchsia-500 font-bold uppercase tracking-wider rounded-xl text-xs text-white transition-colors shadow-lg shadow-fuchsia-500/20 mt-2"
                          >
                            Salvar Opção
                          </button>
                        </form>
                        ...
                        <div className="flex-1 bg-zinc-950/50 p-5 rounded-2xl border border-zinc-800/80">
                          <div className="flex justify-between items-center mb-4">
                            <h4 className="text-zinc-400 font-bold text-xs uppercase tracking-wider">
                              Opções Atuais
                            </h4>
                            <button
                              onClick={limparEnquete}
                              type="button"
                              className="text-rose-400 hover:text-white bg-rose-900/30 hover:bg-rose-600 px-3 py-1.5 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-colors border border-rose-500/30"
                            >
                              Limpar Enquete
                            </button>
                          </div>
                              ...
                              enqueteOpcoes.map((op) => (
                                ...
                                >
                                  ...
                                  <button
                                    ...
                                    className="text-zinc-500 hover:text-rose-400 text-lg transition-colors"
                                    title="Remover"
                                  >
                                    🗑️
                                  </button>
                  ...
                  <details className="group bg-gradient-to-r from-purple-900/20 to-zinc-900 rounded-3xl border border-purple-500/30 shadow-2xl shadow-purple-500/10 [&_summary::-webkit-details-marker]:hidden overflow-hidden">
                    <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer hover:bg-purple-900/10 transition-colors select-none relative">
                      <span className="flex items-center gap-3 relative z-10 text-lg font-black text-purple-400 tracking-tight">
                        🎫 Gerenciar Cupons Promocionais
                      </span>
                      <span className="transition duration-300 group-open:-rotate-180 text-purple-500 relative z-10 text-lg">
                        ▼
                      </span>
                    </summary>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                        ...
                        >
                          <input
                            ...
                            onChange={(e) =>
                              setNovoCupomCodigo(e.target.value.toUpperCase())
                            }
                            className={adminInputClass}
                            required
                          />
                          <div className="flex gap-3">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setNovoCupomValor(e.target.value)
                              }
                              className={adminInputClass}
                              required
                            />
                          ...
                          <button
                            type="submit"
                            className="w-full py-3.5 bg-purple-600 hover:bg-purple-500 font-bold uppercase tracking-wider rounded-xl text-xs text-white transition-colors shadow-lg shadow-purple-500/20 mt-2"
                          >
                            Criar Cupom
                          </button>
                        </form>
                        ...
                        <div className="lg:col-span-2 overflow-y-auto max-h-[200px] pr-3 scrollbar-thin scrollbar-thumb-purple-700 scrollbar-track-transparent">
                          {listaCupons.length === 0 ? (
                            ...
                            <table className="w-full text-left text-sm whitespace-nowrap">
                              <thead>
                                <tr className="text-zinc-400 border-b border-purple-500/30">
                                  <th className="pb-3 font-bold uppercase tracking-wider text-[10px]">
                                    Código
                                  </th>
                                  <th className="pb-3 font-bold uppercase tracking-wider text-[10px]">
                                    Tipo
                                  ...
                                  <th className="pb-3 text-right font-bold uppercase tracking-wider text-[10px]">
                                    Ação
                                  </th>
                                </tr>
                              </thead>
                              <tbody>
                                {listaCupons.map((c) => (
                                  ...
                                  >
                                    <td className="py-4 font-black text-white tracking-widest text-sm">
                                      {c.codigo}
                                    </td>
                                    <td className="py-4 text-zinc-400 text-[10px] font-bold uppercase tracking-wider">
                                      {c.tipo}
                                    </td>
                                    <td className="py-4 text-emerald-400 font-black text-sm">
                                      {c.tipo === "FIXO"
                                        ? `+ R$ ${c.valor.toFixed(2)}`
                                        : `+ ${c.valor}%`}
                                    </td>
                                    <td className="py-4 text-right">
                                      <button
                                        ...
                                        className="text-rose-400 hover:text-white bg-rose-900/30 px-3 py-1.5 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-colors border border-rose-500/30"
                                      >
                                        Excluir
                                      </button>
                                    </td>
                ...
                {contasManutencao.length > 0 && (
                  <section className="bg-rose-950/20 border border-rose-500/50 shadow-2xl shadow-rose-500/10 p-8 rounded-3xl mb-10 animate-pulse-slow">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                      <div>
                        <h3 className="text-xl font-black text-rose-400 tracking-tight mb-2 flex items-center gap-3">
                          🚨 Atenção: Troca de Senha Necessária
                        </h3>
                        <p className="text-xs md:text-sm text-zinc-300 font-medium">
                          As locações abaixo terminaram. Altere a senha na PSN e
                          informe aqui para liberar a conta.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mb-8 flex flex-col md:flex-row gap-4 bg-rose-950/30 p-4 rounded-2xl border border-rose-500/20 shadow-inner">
                      <input
                        ...
                        className={`${adminInputClass} border-rose-500/30 focus:ring-rose-500 flex-1`}
                      />
                      <div className="flex flex-col gap-1 w-full md:w-auto">
                        <select
                          value={ordenacaoManutencao}
                          onChange={(e) =>
                            setOrdenacaoManutencao(e.target.value)
                          }
                          className="bg-zinc-900 border border-rose-500/30 text-zinc-300 text-sm font-bold rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-rose-500 outline-none w-full md:w-56 cursor-pointer hover:border-rose-400 transition-colors h-full"
                        >
                          <option value="urgente">
                            ⚠️ Mais Urgentes (Cashback)
                          </option>
                      ...
                      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        {contasManutencaoFiltradas.map((conta) => (
                          <div
                            ...
                            className="bg-zinc-900 p-6 md:p-8 rounded-3xl border border-rose-500/50 flex flex-col gap-6 shadow-lg"
                          >
                            <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                              <div className="flex flex-col gap-1.5">
                                <strong className="text-white text-lg font-black tracking-tight">
                                  {conta.jogo}
                                </strong>
                                ...
                                <span className="text-xs font-bold text-zinc-500 tracking-wide line-through">
                                  Senha Velha:{" "}
                              ...
                              <div className="flex flex-col gap-3 w-full sm:w-auto">
                                {conta.ultimo_cliente_telefone && (
                                  <button
                                    ...
                                    className="bg-emerald-900/40 hover:bg-emerald-600 text-emerald-400 hover:text-white font-bold uppercase tracking-wider px-4 py-2.5 rounded-xl text-[10px] transition-colors border border-emerald-500/30 shadow flex items-center justify-center gap-2"
                                  >
                                    📱 Cobrar via Whats
                                  </button>
                                )}
                                {conta.ultimo_cliente_id && (
                                  <button
                                    ...
                                    className="bg-rose-900/40 hover:bg-rose-600 text-rose-400 hover:text-white font-bold uppercase tracking-wider px-4 py-2.5 rounded-xl text-[10px] transition-colors border border-rose-500/30 shadow flex items-center justify-center gap-2"
                                  >
                                    🚨 Aplicar Multa
                                  </button>
                                )}
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 mt-2 pt-6 border-t border-rose-900/50">
                              <input
                                ...
                                value={
                                  novasSenhasTemp[conta.conta_psn_id] || ""
                                }
                                onChange={(e) =>
                                  setNovasSenhasTemp({
                                    ...novasSenhasTemp,
                                ...
                                className="flex-1 px-5 py-3.5 bg-zinc-950 border border-zinc-800 rounded-2xl text-sm font-bold text-white focus:border-rose-500 outline-none"
                              />
                              <button
                                onClick={() =>
                                  confirmarResetSenha(conta.conta_psn_id)
                                }
                                className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold uppercase tracking-wider px-6 py-3.5 rounded-2xl text-xs transition-colors shadow-lg shadow-emerald-600/20 whitespace-nowrap"
                              >
                                Liberar Jogo
                              </button>
                            </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 mb-10">
                  ...
                  <div className="bg-gradient-to-br from-blue-900/20 to-zinc-900 rounded-3xl border border-blue-500/30 p-8 shadow-2xl shadow-blue-500/10 flex flex-col">
                    <h3 className="text-lg font-black text-blue-400 tracking-tight mb-8 flex items-center gap-3">
                      ➕ Cadastrar Novo Jogo
                    </h3>
                    <form
                    ...
                    >
                      <div className="flex gap-3">
                        ...
                        <button
                          ...
                          className="bg-amber-500 hover:bg-amber-400 text-zinc-900 font-bold px-5 rounded-xl text-[10px] uppercase tracking-wider whitespace-nowrap transition-colors shadow-lg shadow-amber-500/20"
                        >
                          ✨ Buscar
                        </button>
                      </div>
                      ...
                      <div className="flex flex-col md:flex-row gap-4">
                        <div className="w-full">
                          <select
                            ...
                            onChange={(e) =>
                              setNovoJogoPlataforma(e.target.value)
                            }
                            className={adminInputClass}
                          >
                            <option value="PS5">PS5</option>
                        ...
                        <div className="w-full relative">
                          <label className="absolute -top-2 left-3 bg-zinc-900 px-1 text-[10px] font-bold text-zinc-500 uppercase tracking-wider">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setNovoJogoDataLancamento(e.target.value)
                            }
                            className={adminInputClass}
                          />
                        </div>
                      ...
                      <button
                        type="submit"
                        className="w-full mt-auto py-4 bg-blue-600 hover:bg-blue-500 text-white font-bold uppercase tracking-wider rounded-xl text-xs transition-colors shadow-lg shadow-blue-500/20"
                      >
                        Salvar no Catálogo
                      </button>
                    </form>
                  ...
                  <div className="bg-gradient-to-br from-fuchsia-900/20 to-zinc-900 rounded-3xl border border-fuchsia-500/30 p-8 shadow-2xl shadow-fuchsia-500/10 flex flex-col">
                    <h3 className="text-lg font-black text-fuchsia-400 tracking-tight mb-8 flex items-center gap-3">
                      📦 Abastecer Estoque
                    </h3>
                    <input
                      type="text"
                    ...
                    >
                      ...
                      <button
                        type="submit"
                        className="w-full mt-auto py-4 bg-fuchsia-600 hover:bg-fuchsia-500 text-white font-bold uppercase tracking-wider rounded-xl text-xs transition-colors shadow-lg shadow-fuchsia-500/20"
                      >
                ...
                <div className="flex flex-col gap-8 mb-10">
                  ...
                  <details className="group bg-zinc-900/80 rounded-3xl border border-zinc-800 border-l-4 border-l-blue-500 shadow-2xl shadow-blue-500/10 [&_summary::-webkit-details-marker]:hidden overflow-hidden">
                    <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer hover:bg-blue-900/10 transition-colors select-none">
                      <span className="flex items-center gap-3 text-lg font-black text-blue-400 tracking-tight">
                        🎮 Catálogo de Jogos ({jogos.length})
                      </span>
                      <span className="transition duration-300 group-open:-rotate-180 text-blue-500 text-lg">
                        ▼
                      </span>
                    </summary>
                    <div className="px-6 md:px-8 pb-6 md:pb-8 border-t border-zinc-800/50 pt-8">
                      ...
                      <div className="mb-6 flex flex-col lg:flex-row gap-4 bg-zinc-950 p-4 rounded-2xl border border-zinc-800/80 shadow-inner items-center">
                        ...
                        <div className="flex bg-zinc-900 rounded-xl p-1 border border-zinc-700/50 shadow-inner w-full lg:w-auto overflow-x-auto">
                          <button
                            ...
                            className={`flex-1 lg:flex-none px-4 py-2 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-all ${filtroStatusCatalogo === "todos" ? "bg-blue-600 text-white" : "text-zinc-400 hover:text-white"}`}
                          >
                            Todos
                          </button>
                          <button
                            onClick={() =>
                              setFiltroStatusCatalogo("disponiveis")
                            }
                            className={`flex-1 lg:flex-none px-4 py-2 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-all ${filtroStatusCatalogo === "disponiveis" ? "bg-emerald-600 text-white" : "text-zinc-400 hover:text-white"}`}
                          >
                            Livres
                          </button>
                          <button
                            onClick={() => setFiltroStatusCatalogo("alugados")}
                            className={`flex-1 lg:flex-none px-4 py-2 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-all ${filtroStatusCatalogo === "alugados" ? "bg-rose-600 text-white" : "text-zinc-400 hover:text-white"}`}
                          >
                            Alugados
                          </button>
                          <button
                            onClick={() =>
                              setFiltroStatusCatalogo("lancamentos")
                            }
                            className={`flex-1 lg:flex-none px-4 py-2 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-all ${filtroStatusCatalogo === "lancamentos" ? "bg-fuchsia-600 text-white" : "text-zinc-400 hover:text-white"}`}
                          >
                            🔥 Lançam.
                          </button>
                        </div>
                          ...
                          <ul className="space-y-3">
                            {jogosCatalogoAdminFiltrados
                              ...
                              .map((jogo) => (
                                <li
                                  key={jogo.id}
                                  className="flex flex-col md:flex-row justify-between items-start md:items-center bg-zinc-950/50 p-4 md:p-5 rounded-2xl border-l-2 border-blue-500 gap-4 shadow-sm hover:bg-zinc-800/50 transition-colors"
                                >
                                  <div className="flex flex-col leading-relaxed gap-1 w-full md:w-auto">
                                    <span className="font-black text-sm text-white tracking-tight truncate max-w-[300px]">
                                      {jogo.titulo}
                                    </span>
                                    <div className="flex items-center gap-3">
                                      <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">
                                      ...
                                      <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">
                                        14D: R${" "}
                                        {jogo.preco_aluguel_14.toFixed(2)}
                                      </span>
                                    </div>
                                    {jogo.estoque > 0 ? (
                                      <span className="text-emerald-400 text-[10px] font-bold uppercase tracking-wider mt-1">
                                        ✅ {jogo.estoque} Disponível
                                      </span>
                                    ) : (
                                      <span className="text-rose-400 text-[10px] font-bold uppercase tracking-wider mt-1">
                                        ❌ Alugado
                                      </span>
                                    )}
                                  </div>
                                  ...
                                  <div className="flex gap-2 w-full md:w-auto justify-end">
                                    <button
                                      ...
                                      className="text-blue-400 hover:text-white text-[10px] uppercase tracking-wider bg-blue-900/30 hover:bg-blue-600 px-4 py-2 rounded-lg font-bold transition-colors border border-blue-500/30"
                                    >
                                      Editar
                                    </button>
                                    <button
                                      onClick={() => removerJogo(jogo.id)}
                                      className="text-rose-400 hover:text-white text-[10px] uppercase tracking-wider bg-rose-900/30 hover:bg-rose-600 px-4 py-2 rounded-lg font-bold transition-colors border border-rose-500/30"
                                    >
                                      Excluir
                                    </button>
                                  </div>
                      ...
                      <div className="mt-6 flex justify-between items-center bg-zinc-950 p-4 rounded-2xl border border-zinc-800/80">
                        <button
                          onClick={() =>
                            setPaginaCatalogo(Math.max(0, paginaCatalogo - 1))
                          }
                          disabled={paginaCatalogo === 0}
                          className="px-5 py-2.5 bg-zinc-800 text-white rounded-xl disabled:opacity-50 hover:bg-zinc-700 text-[10px] uppercase tracking-wider font-bold transition-colors"
                        >
                          ◀ Anterior
                        </button>
                        ...
                        <button
                          ...
                          disabled={
                            (paginaCatalogo + 1) * 50 >=
                            jogosCatalogoAdminFiltrados.length
                          }
                          className="px-5 py-2.5 bg-zinc-800 text-white rounded-xl disabled:opacity-50 hover:bg-zinc-700 text-[10px] uppercase tracking-wider font-bold transition-colors"
                        >
                          Próxima ▶
                    ...
                    <div className="px-6 md:px-8 pb-6 md:pb-8 border-t border-zinc-800/50 pt-8">
                      ...
                      <div className="mb-6 flex flex-col md:flex-row gap-4 bg-zinc-950 p-4 rounded-2xl border border-zinc-800/80 shadow-inner">
                        ...
                        <select
                          ...
                          className="bg-zinc-900 border border-emerald-500/30 text-zinc-300 text-sm font-bold rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none w-full md:w-56 cursor-pointer hover:border-emerald-400 transition-colors"
                        >
                          <option value="expira_breve">
                            ⏳ Expira em Breve
                          </option>
                            ...
                            <tbody>
                              {locacoesAtivasFiltradas.map((loc) => (
                                ...
                                >
                                  ...
                                  <td className="py-4 font-black text-white text-sm tracking-tight">
                                    {loc.jogo}
                                  </td>
                                  ...
                                  <td className="py-4">
                                    <div className="flex justify-end gap-2">
                                      <button
                                        onClick={() =>
                                          avisarLiberacao(loc.cliente, loc.jogo)
                                        }
                                        className="text-emerald-400 hover:text-white text-[10px] uppercase tracking-wider bg-emerald-900/30 hover:bg-emerald-600 px-3 py-1.5 rounded-lg font-bold transition-colors border border-emerald-500/30 shadow flex items-center gap-1.5"
                                      ...
                                      <button
                                        ...
                                        className="text-rose-400 hover:text-white text-[10px] uppercase tracking-wider bg-rose-900/30 hover:bg-rose-600 px-3 py-1.5 rounded-lg font-bold transition-colors border border-rose-500/30 shadow"
                                      >
                                        Revogar
                                      </button>
                                    </div>
                    ...
                    <div className="px-6 md:px-8 pb-6 md:pb-8 border-t border-zinc-800/50 pt-8">
                      <div className="mb-6 flex flex-col md:flex-row gap-4 bg-zinc-950 p-4 rounded-2xl border border-zinc-800/80 shadow-inner">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setOrdenacaoReservaAdmin(e.target.value)
                          }
                          className="bg-zinc-900 border border-amber-500/30 text-zinc-300 text-sm font-bold rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-amber-500 outline-none w-full md:w-56 cursor-pointer hover:border-amber-400 transition-colors"
                        >
                            ...
                            <tbody>
                              {reservasAdminFiltradas.map((reserva) => (
                                ...
                                >
                                  ...
                                  <td className="py-4 text-blue-400 text-xs font-bold">
                                    {reserva.data_inicio}
                                  </td>
                                  <td className="py-4 text-amber-400 text-xs font-bold">
                                    {reserva.data_fim}{" "}
                  ...
                  <details className="group bg-zinc-900/80 rounded-3xl border border-zinc-800 border-l-4 border-l-purple-500 shadow-2xl shadow-purple-500/10 [&_summary::-webkit-details-marker]:hidden overflow-hidden">
                    <summary className="flex items-center justify-between p-6 md:p-8 cursor-pointer hover:bg-purple-900/10 transition-colors select-none">
                      <span className="flex items-center gap-3 text-lg font-black text-purple-400 tracking-tight">
                    ...
                    <div className="px-6 md:px-8 pb-6 md:pb-8 border-t border-zinc-800/50 pt-8">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                          ...
                          <div className="flex bg-zinc-900 rounded-xl p-1 border border-zinc-700/50 shadow-inner w-full sm:w-auto">
                            <button
                              ...
                              className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-all ${filtroSaldoClientes === "todos" ? "bg-purple-600 text-white shadow-md" : "text-zinc-400 hover:text-white hover:bg-zinc-800"}`}
                            >
                              Todos
                            </button>
                            <button
                              ...
                              className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-all ${filtroSaldoClientes === "positivo" ? "bg-emerald-600 text-white shadow-md" : "text-zinc-400 hover:text-white hover:bg-zinc-800"}`}
                            >
                              Positivos
                            </button>
                            <button
                              ...
                              className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-[10px] uppercase tracking-wider font-bold transition-all ${filtroSaldoClientes === "negativo" ? "bg-rose-600 text-white shadow-md" : "text-zinc-400 hover:text-white hover:bg-zinc-800"}`}
                            >
                              Negativados
                            </button>
                          </div>
                          ...
                          <ul className="space-y-4">
                            {clientesFiltrados
                              ...
                              .map((u) => (
                                <li
                                  key={u.id}
                                  className="flex flex-col md:flex-row justify-between items-start md:items-center bg-zinc-950/50 p-4 md:p-5 rounded-2xl border border-zinc-800/50 border-l-2 border-l-purple-500 shadow-sm hover:bg-zinc-800/50 transition-colors gap-4"
                                >
                                  <div className="flex flex-col gap-1.5">
                                    <span className="text-sm font-black text-white tracking-tight">
                                      {u.nome}{" "}
                                      {u.is_admin && (
                                        <span className="ml-2 text-[8px] bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded-md uppercase tracking-wider">
                                          Admin
                                        </span>
                                      )}
                                    </span>
                                    <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">
                                      Saldo:{" "}
                                      <strong
                                        className={`text-xs tracking-normal ml-1 ${u.saldo < 0 ? "text-rose-400" : "text-emerald-400"}`}
                                      >
                                  ...
                                  {!u.is_admin && (
                                    <div className="flex flex-wrap gap-2 w-full md:w-auto justify-end mt-2 md:mt-0">
                                      ...
                                      <button
                                        ...
                                        className="text-blue-400 hover:text-white text-[10px] uppercase tracking-wider bg-blue-900/30 hover:bg-blue-600 px-3 py-1.5 rounded-lg font-bold transition-colors border border-blue-500/30 flex items-center gap-1"
                                      >
                                        ✏️ Editar
                                      </button>
                                      <button
                                        onClick={() => removerUsuario(u.id)}
                                        className="text-rose-400 hover:text-white text-[10px] uppercase tracking-wider bg-rose-900/30 hover:bg-rose-600 px-3 py-1.5 rounded-lg font-bold transition-colors border border-rose-500/30 flex items-center gap-1"
                                      >
                                        🗑️ Excluir
                                      </button>
                                    </div>
                      ...
                      <div className="mt-6 flex justify-between items-center bg-zinc-950 p-4 rounded-2xl border border-zinc-800/80">
                        <button
                          onClick={() =>
                            setPaginaClientes(Math.max(0, paginaClientes - 1))
                          }
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
                <div className="col-span-1 md:col-span-2">
                  <span className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-emerald-400 tracking-tighter mb-4 block">
                    BORA JOGAR!
                  </span>
                  <p className="text-zinc-400 text-sm leading-relaxed max-w-sm mb-8 font-medium">
                    Sua locadora de jogos digitais next-gen. Alugue os maiores
                    lançamentos de PlayStation de forma automática, rápida e sem
                    sair de casa.
                  </p>
                  <div className="flex gap-4">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-12 h-12 rounded-xl bg-zinc-950 border border-zinc-800 flex items-center justify-center text-zinc-400 hover:text-white hover:border-pink-500 hover:bg-pink-500/10 transition-all shadow-lg text-lg"
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-12 h-12 rounded-xl bg-zinc-950 border border-zinc-800 flex items-center justify-center text-zinc-400 hover:text-white hover:border-emerald-500 hover:bg-emerald-500/10 transition-all shadow-lg text-lg"
                ...
                <div>
                  <h4 className="text-white font-bold mb-6 tracking-wider uppercase text-[10px]">
                    Acesso Rápido
                  </h4>
